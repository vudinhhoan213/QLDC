...
module.exports = {
  ...
  async getAll(req, res, next) {
    try {
      ...
      const data = await rewardProposalService.getAll(filter, { page: Number(page) || 1, limit: Number(limit) || 50, sort });
      res.json(data);
    } catch (err) { next(err); }
  },
  ...
  async approve(req, res, next) {
    try {
      ...
      const result = await rewardProposalService.approveRewardProposal({ id: req.params.id, reviewerUserId: req.user._id });
      res.json(result);
    } catch (err) { next(err); }
  },
  ...
  async reject(req, res, next) {
    try {
      ...
      const result = await rewardProposalService.rejectRewardProposal({ id: req.params.id, reviewerUserId: req.user._id, rejectionReason: reason });
      res.json(result);
    } catch (err) { next(err); }
  },
};

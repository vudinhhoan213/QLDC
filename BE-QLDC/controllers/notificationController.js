...
module.exports = {
  ...
  async getAll(req, res, next) {
    try {
      ...
      const data = await notificationService.getAll(filter, { page: Number(page) || 1, limit: Number(limit) || 50, sort });
      res.json(data);
    } catch (err) { next(err); }
  },
  ...
  async markAsRead(req, res, next) {
    try {
      const doc = await notificationService.markAsRead(req.params.id, req.user && req.user._id);

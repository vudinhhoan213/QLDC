...
module.exports = {
  ...
  async getAll(filter = {}, options = {}) {
    ...
    const docs = await EditRequest.find(filter)
      .populate('citizen requestedBy reviewedBy')
      .sort(sort)
      .limit(limit)
      .skip((page - 1) * limit);
  ...
  async getById(id) {
    return EditRequest.findById(id).populate('citizen requestedBy reviewedBy');
  },
  ...
  async approveEditRequest({ id, reviewerUserId }) {
    const reqDoc = await EditRequest.findById(id).populate('citizen requestedBy');
    if (!reqDoc) throw new Error('EditRequest not found');
    if (reqDoc.status !== 'PENDING') throw new Error('EditRequest is not pending');
